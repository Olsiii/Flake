-- Adds city and keyword params to the /search RPC functions, backing the
-- natural-language search box on /search: structured extraction (via
-- /api/ai-search) fills p_city along with the existing numeric/enum filters,
-- while the fallback path (AI call failed or returned nothing usable) sends
-- the raw query as p_keyword and matches it loosely against
-- title/description/city.
--
-- CREATE OR REPLACE can't widen a function's parameter list — Postgres
-- treats a changed arg count as a distinct overload rather than a true
-- replace, which left two ambiguous search_listings_bbox/polygon
-- definitions the first time this ran. Drop the old signatures first so
-- the CREATE OR REPLACE below lands as the only definition.
drop function if exists search_listings_bbox(
  double precision, double precision, double precision, double precision,
  numeric, numeric, smallint, numeric, property_type[],
  integer, integer, smallint, smallint, boolean, text, integer
);
drop function if exists search_listings_polygon(
  text, numeric, numeric, smallint, numeric, property_type[],
  integer, integer, smallint, smallint, boolean, text, integer
);
